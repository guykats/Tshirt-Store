...
**Watch out:** a fresh `migrate:fresh --seed --force` only ever replays git-tracked migration history — it has no idea about an "Approve for development" click made directly on the live production dashboard, since that click writes straight to production's real database and nothing else reads it. In `pm-agent.yml`, the "Sync real approved-task titles from production" step already corrects this before the PM Agent starts (see `PmAgentAutomationController::approvedTodoTitles` / `app/Console/Commands/SyncApprovedTaskTitles.php`), so a scheduled run's view of `approved_for_dev` is real. An *interactive* session has no such step — if the owner tells you they approved something, trust them and encode it into a migration yourself (as this project has done throughout its history) rather than assuming a bare `migrate:fresh --seed --force` already reflects it.
