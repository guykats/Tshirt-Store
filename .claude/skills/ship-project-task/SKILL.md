...
**Watch out:** a fresh `migrate:fresh --seed --force` only ever replays git-tracked migration history — it has no idea about an "Approve for development" click, or an epic approve/reject/delay decision, made directly on the live production dashboard, since those writes go straight to production's real database and nothing else reads it. Epics have an extra wrinkle: they can also be *created* live with no migration at all (the `/dashboard/chat` Visioner conversation, `VisionerChatController::proposeEpic`), so a replay can be missing a row entirely, not just have its status wrong. In `pm-agent.yml`, the "Sync real approved-task titles from production" and "Sync real epic decisions from production" steps already correct both before the PM Agent starts (see `PmAgentAutomationController` / `app/Console/Commands/SyncApprovedTaskTitles.php` / `SyncEpicDecisions.php`), so a scheduled run's view of both tables is real. An *interactive* session has no such step — if the owner tells you they approved a task or decided an epic, trust them and encode it into a migration yourself (as this project has done throughout its history) rather than assuming a bare `migrate:fresh --seed --force` already reflects it. If you break a synced-in, migration-less epic into tasks, write the epic's own insert migration first (mirroring its real title/description/status) before referencing its id anywhere else, or the reference will dangle on the next fresh replay.
