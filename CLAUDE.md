...
- **`deploy.yml` silently stopped auto-deploying on 2026-08-01 — still broken
  ...
  2026-08-13 (last re-checked ~14:44 UTC, still zero drift vs. the ~11:38 UTC
  snapshot below — same deploy timestamp/sha (`2026-08-01T18:20:33Z`/
  `8dde7500`), same unverified HEAD, same 91 unapproved backlog, no
  `actions: write` grant, no PAT; this check found all 12 epics still at
  `proposed` (including 7, 9, 15, 16, 18) and task 348 still `todo`/
  unapproved, the same task-348 oscillation already documented below, not a
  new finding — still no approved-epic-awaiting-breakdown work and no
  approved todo task to build this run either) has re-checked
