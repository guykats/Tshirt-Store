...
- **`deploy.yml` silently stopped auto-deploying on 2026-08-01 and is STILL
  ...
  **Re-check (2026-08-19, ~05:01 UTC, >12h later per rule (b)): still zero
  drift** — same sha `8dde7500` (2026-08-01T18:20:33Z), HEAD unverified, no
  `actions: write` grant, no PAT, backlog still 91, epics 7/9/15/16/18 all
  `approved` (no oscillation this time), task 348 still `todo`/unapproved,
  nothing approved to build. **Re-check (2026-08-19, ~17:16 UTC, >12h later
  per rule (b)): still zero drift** — same sha `8dde7500`
  (2026-08-01T18:20:33Z), HEAD unverified, no `actions: write` grant, no
  PAT, backlog still 91, epics 7/9/15/16/18 all `approved` (no oscillation
  this time), task 348 still `todo`/unapproved, no approved `todo` task and
  no approved epic-awaiting-breakdown existed to build (all 5 approved
  epics already have ≥1 linked task). **Re-check (2026-08-20, ~05:43 UTC,
  >12h later per rule (b)): still zero drift** — same sha `8dde7500`
  (2026-08-01T18:20:33Z), HEAD (`2216ecd`, 2026-08-19T17:17:14Z) unverified,
  ...
  "optional, not mandatory" guidance above. **Re-check (2026-08-21, ~05:54
  UTC, exactly 12h later per rule (b)): still zero drift on the deploy
  itself** — last successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push (confirmed via `gh run list --workflow=deploy.yml`),
  HEAD (`ad8244c`) unverified, no `actions: write` grant (a live
  `gh workflow run deploy.yml --ref main` still fails with the same
  `HTTP 403: Resource not accessible by integration`), no PAT, backlog
  still 91 unapproved `todo` tasks, task 345 still `blocked`/correct, task
  348 still `todo`/unapproved. Epics 7/9/15/16/18 flipped back to
  `approved` this check (reversing the 17:54 UTC flip to `proposed`, linked
  `done` tasks intact throughout) — another oscillation event per task 348,
  folded in here under the same >12h rule rather than as its own commit.
  No approved `todo` task and no approved epic-awaiting-breakdown existed
  to build (all 5 approved epics already have ≥1 linked task), so nothing
  was shippable this run; the 91-item unapproved backlog was left as-is per
  the "optional, not mandatory" guidance above. **Re-check (2026-08-21,
  ~18:20 UTC, >12h later per rule (b)): still zero drift** — last
  successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push (confirmed via `gh run list --workflow=deploy.yml`),
  HEAD (`b56021c`) unverified, no `actions: write` grant (a live
  `gh workflow run deploy.yml --ref main` still fails with the same
  `HTTP 403: Resource not accessible by integration`), no PAT, backlog
  still 91 unapproved `todo` tasks, task 345 still `blocked`/correct, task
  348 still `todo`/unapproved. Epics 7/9/15/16/18 all `approved` this check
  ...
  mandatory" guidance above. **Re-check (2026-08-22, ~11:35 UTC, >12h later
  per rule (b)): still zero drift** — last successful `Deploy to
  Production` run is still the same 2026-08-01T18:20:33Z push (confirmed
  via `gh run list --workflow=deploy.yml`), HEAD (`2f42808`) unverified, no
  ...
  integration`), no PAT (`gh secret list` also 403s, consistent with no
  `actions: write`), backlog still 91 unapproved `todo` tasks, task 345
  still `blocked`/correct, task 348 still `todo`/unapproved. Epics
  7/9/15/16/18 flipped back to `approved` this check (reversing the 23:14
  UTC check's `proposed` reading, linked `done` tasks intact throughout) —
  another oscillation event per task 348, folded in here under the same
  >12h rule rather than as its own commit. No approved `todo` task existed
  and no approved epic-awaiting-breakdown existed to build (all 5 approved
  epics already have ≥1 linked task), so nothing was shippable this run;
  the 91-item unapproved backlog was left as-is per the "optional, not
  mandatory" guidance above. **Re-check (2026-08-22, ~23:55 UTC, >12h later
  per rule (b)): still zero drift** — last successful `Deploy to
  Production` run is still the same 2026-08-01T18:20:33Z push (confirmed
  via `gh run list --workflow=deploy.yml`), HEAD (`594d6a5`) unverified, no
  `actions: write` grant (a live `gh workflow run deploy.yml --ref main`
  still fails with the same `HTTP 403: Resource not accessible by
  ...
  `todo`/unapproved. Epics 7/9/15/16/18 all still `approved` this check (no
  oscillation since the 11:35 UTC check). No approved `todo` task and no
  ...
  shippable; the 91-item unapproved backlog was left as-is per the
  "optional, not mandatory" guidance above. **Re-check (2026-08-23, ~14:21
  UTC, >12h later per rule (b)): still zero drift** — last successful
  `Deploy to Production` run is still the same 2026-08-01T18:20:33Z push
  (confirmed via `gh run list --workflow=deploy.yml`), HEAD (`c9b12e7`)
  unverified, no `actions: write` grant (a live `gh workflow run
  deploy.yml --ref main` still fails with the same `HTTP 403: Resource not
  accessible by integration`), no PAT (`gh secret list` also 403s),
  backlog still 91 unapproved `todo` tasks, task 345 still
  `blocked`/correct, task 348 still `todo`/unapproved. Epics 7/9/15/16/18
  flipped back to `approved` this check (reversing the 02:14 UTC check's
  `proposed` reading, linked `done` tasks intact throughout) — another
  oscillation event per task 348, folded in here under the same >12h rule
  rather than as its own commit. No approved `todo` task existed and no
  approved epic-awaiting-breakdown existed to build (all 5 approved epics
  already have ≥1 linked task), so nothing was shippable this run; the
  91-item unapproved backlog was left as-is per the "optional, not
  mandatory" guidance above. **Re-check (2026-08-24, ~03:15 UTC, >12h later
  per rule (b)): still zero drift** — last successful `Deploy to
  Production` run is still the same 2026-08-01T18:20:33Z push (confirmed
  via `gh run list --workflow=deploy.yml`), HEAD (`5e541d1`) unverified, no
  `actions: write` grant (a live `gh workflow run deploy.yml --ref main`
  still fails with the same `HTTP 403: Resource not accessible by
  integration`), no PAT (`gh secret list` also 403s), backlog still 91
  unapproved `todo` tasks, task 345 still `blocked`/correct, task 348 still
  `todo`/unapproved. Epics 7/9/15/16/18 all still `approved` this check (no
  oscillation since the 14:21 UTC check). No approved `todo` task and no
  approved epic-awaiting-breakdown existed to build (all 5 approved epics
  already have ≥1 linked task), so nothing was shippable this run; the
  91-item unapproved backlog was left as-is per the "optional, not
  mandatory" guidance above. **Re-check (2026-08-24, ~08:11 UTC): still
  zero drift on the deploy itself** — last successful `Deploy to
  Production` run is still the same 2026-08-01T18:20:33Z push (confirmed
  via `gh run list --workflow=deploy.yml`), HEAD (`8e7d9a8`) unverified, no
  `actions: write` grant (a live `gh workflow run deploy.yml --ref main`
  still fails with the same `HTTP 403: Resource not accessible by
  integration`), no PAT (`gh secret list` also 403s), backlog still 91
  unapproved `todo` tasks, task 345 still `blocked`/correct, task 348 still
  `todo`/unapproved. Epics 7/9/15/16/18 flipped back to `proposed` this
  check (reversing the 03:15 UTC check's `approved` reading, linked `done`
  tasks intact throughout) — another oscillation event per task 348. This
  is being folded into a commit now under the narrowed rule because it's
  >12h since the last *recorded oscillation note* (2026-08-23 ~14:21 UTC),
  even though it's only ~5h since the last routine zero-drift timestamp
  (~03:15 UTC same day) — the two triggers are tracked independently, and
  this run's commit is justified by the oscillation-note clock, not the
  routine-recheck clock, exactly as in the 2026-08-21 ~23:14 UTC and
  2026-08-23 ~02:14 UTC precedents. No approved `todo` task existed and no
  ...
  **Re-check (2026-08-24, ~20:16 UTC, >12h later per rule (b)): still zero
  drift on the deploy itself** — last successful `Deploy to Production` run
  is still the same 2026-08-01T18:20:33Z push (confirmed via `gh run list
  --workflow=deploy.yml`), HEAD (`05f043b`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 flipped back to `approved` this check (reversing the
  08:11 UTC check's `proposed` reading, linked `done` tasks intact
  throughout) — another oscillation event per task 348, folded in here
  under the same >12h rule (last recorded oscillation note was also
  2026-08-24 ~08:11 UTC) rather than as its own commit. No approved `todo`
  task existed and no approved epic-awaiting-breakdown existed to build
  (all 5 approved epics already have ≥1 linked task), so nothing was
  shippable this run; the 91-item unapproved backlog was left as-is per
  the "optional, not mandatory" guidance above. **Re-check (2026-08-25,
  ~08:29 UTC, >12h later per rule (b)): still zero drift** — last
  successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run view`
  on that exact run id), HEAD (`a4c1168`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 all still `approved` this check (no oscillation since
  the 20:16 UTC check). No approved `todo` task and no approved
  ...
  guidance above. **Re-check (2026-08-25, ~20:47 UTC, >12h later per rule
  (b)): still zero drift** — last successful `Deploy to Production` run is
  still the same 2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via
  `gh run list --workflow=deploy.yml`), HEAD (`56734d1`) unverified, no
  `actions: write` grant (a live `gh workflow run deploy.yml --ref main`
  still fails with the same `HTTP 403: Resource not accessible by
  integration`), no PAT (`gh secret list` also 403s), backlog still 91
  unapproved `todo` tasks, task 345 still `blocked`/correct, task 348 still
  `todo`/unapproved. Epics 7/9/15/16/18 all still `approved` this check (no
  oscillation since the 08:29 UTC check). No approved `todo` task and no
  approved epic-awaiting-breakdown existed to build (all 5 approved epics
  already have ≥1 linked task), so nothing was shippable this run; the
  91-item unapproved backlog was left as-is per the "optional, not
  ...
  left as-is per the "optional, not mandatory" guidance above. **Re-check
  (2026-08-26, ~20:18 UTC, >12h later per rule (b)): still zero drift** —
  last successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml`), HEAD (`dc8d446`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 flipped back to `approved` this check (reversing the
  06:31 UTC check's `proposed` reading, linked `done` tasks intact
  throughout) — another oscillation event per task 348, folded in here
  under the same >12h rule rather than as its own commit. No approved
  `todo` task and no approved epic-awaiting-breakdown existed to build
  (all 5 approved epics already have ≥1 linked task), so nothing was
  shippable this run; the 91-item unapproved backlog was left as-is per
  the "optional, not mandatory" guidance above. **Re-check (2026-08-27,
  ~16:37 UTC, >12h later per rule (b)): still zero drift** — last
  successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml`), HEAD (`b8dcc5d`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 all still `approved` this check (no oscillation since
  the 20:18 UTC check). No approved `todo` task and no approved
  epic-awaiting-breakdown existed to build (all 5 approved epics already
  have ≥1 linked task), so nothing was shippable this run; the 91-item
  unapproved backlog was left as-is per the "optional, not mandatory"
  guidance above. **Re-check (2026-08-28, ~12:44 UTC, >12h later per rule
  (b)): still zero drift** — last successful `Deploy to Production` run is
  still the same 2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via
  `gh run list --workflow=deploy.yml`), HEAD (`28d8c9b`) unverified, no
  `actions: write` grant (a live `gh workflow run deploy.yml --ref main`
  still fails with the same `HTTP 403: Resource not accessible by
  integration`), no PAT (`gh secret list` also 403s), backlog still 91
  unapproved `todo` tasks, task 345 still `blocked`/correct, task 348 still
  `todo`/unapproved. Epics 7/9/15/16/18 all still `approved` this check (no
  oscillation since the 16:37 UTC check). No approved `todo` task and no
  approved epic-awaiting-breakdown existed to build (all 5 approved epics
  already have ≥1 linked task), so nothing was shippable this run; the
  91-item unapproved backlog was left as-is per the "optional, not
  mandatory" guidance above. **Re-check (2026-08-29, ~03:52 UTC, >12h later
  per rule (b)): still zero drift** — last successful `Deploy to
  Production` run is still the same 2026-08-01T18:20:33Z push at sha
  `8dde7500` (confirmed via `gh run list --workflow=deploy.yml`), HEAD
  (`1435303`) unverified, no `actions: write` grant (a live `gh workflow
  ...
  backlog still 91 unapproved `todo` tasks, task 345 still
  `blocked`/correct, task 348 still `todo`/unapproved. Epics 7/9/15/16/18
  all still `approved` this check (no oscillation since the 12:44 UTC
  check). No approved `todo` task and no approved epic-awaiting-breakdown
  existed to build (all 5 approved epics already have ≥1 linked task), so
  nothing was shippable this run; the 91-item unapproved backlog was left
  as-is per the "optional, not mandatory" guidance above. **Re-check
  (2026-08-29, ~18:41 UTC, >12h later per rule (b)): still zero drift** —
  last successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml`), HEAD (`2ef93e6`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 all still `approved` this check (no oscillation since
  the 03:52 UTC check). No approved `todo` task and no approved
  epic-awaiting-breakdown existed to build (all 5 approved epics already
  have ≥1 linked task), so nothing was shippable this run; the 91-item
  unapproved backlog was left as-is per the "optional, not mandatory"
  guidance above. **Re-check (2026-08-30, ~06:52 UTC, >12h later per rule
  (b)): still zero drift** — last successful `Deploy to Production` run is
  still the same 2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via
  `gh run list --workflow=deploy.yml`), HEAD (`0aac384`) unverified, no
  `actions: write` grant (a live `gh workflow run deploy.yml --ref main`
  still fails with the same `HTTP 403: Resource not accessible by
  integration`), no PAT (`gh secret list` also 403s), backlog still 91
  unapproved `todo` tasks, task 345 still `blocked`/correct, task 348 still
  `todo`/unapproved. Epics 7/9/15/16/18 all still `approved` this check (no
  oscillation since the 18:41 UTC check). No approved `todo` task and no
  approved epic-awaiting-breakdown existed to build (all 5 approved epics
  already have ≥1 linked task), so nothing was shippable this run; the
  91-item unapproved backlog was left as-is per the "optional, not
  mandatory" guidance above. **Re-check (2026-08-30, ~19:33 UTC, >12h later
  per rule (b)): still zero drift** — last successful `Deploy to
  Production` run is still the same 2026-08-01T18:20:33Z push at sha
  `8dde7500` (confirmed via `gh run list --workflow=deploy.yml`), HEAD
  (`1a226eb`) unverified, no `actions: write` grant (a live `gh workflow
  run deploy.yml --ref main` still fails with the same `HTTP 403: Resource
  not accessible by integration`), no PAT (`gh secret list` also 403s),
  backlog still 91 unapproved `todo` tasks, task 345 still
  `blocked`/correct, task 348 still `todo`/unapproved. Epics 7/9/15/16/18
  all still `approved` this check (no oscillation since the 06:52 UTC
  check). No approved `todo` task and no approved epic-awaiting-breakdown
  existed to build (all 5 approved epics already have ≥1 linked task), so
  nothing was shippable this run; the 91-item unapproved backlog was left
  as-is per the "optional, not mandatory" guidance above. **Re-check
  (2026-08-31, ~13:39 UTC, >12h later per rule (b)): still zero drift** —
  last successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml`), HEAD (`c043dd1`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 all still `approved` this check (no oscillation since
  the 19:33 UTC check). No approved `todo` task and no approved
  epic-awaiting-breakdown existed to build (all 5 approved epics already
  have ≥1 linked task), so nothing was shippable this run; the 91-item
  unapproved backlog was left as-is per the "optional, not mandatory"
  guidance above. **Re-check (2026-09-01, ~01:54 UTC, >12h later per rule
  (b)): still zero drift** — last successful `Deploy to Production` run is
  still the same 2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via
  `gh run list --workflow=deploy.yml`), HEAD (`ea068c4`) unverified, no
  `actions: write` grant (a live `gh workflow run deploy.yml --ref main`
  still fails with the same `HTTP 403: Resource not accessible by
  integration`), no PAT (`gh secret list` also 403s), backlog still 91
  unapproved `todo` tasks, task 345 still `blocked`/correct, task 348 still
  `todo`/unapproved. Epics 7/9/15/16/18 all still `approved` this check (no
  oscillation since the 13:39 UTC check). No approved `todo` task and no
  approved epic-awaiting-breakdown existed to build (all 5 approved epics
  already have ≥1 linked task), so nothing was shippable this run; the
  91-item unapproved backlog was left as-is per the "optional, not
  mandatory" guidance above. **Re-check (2026-09-01, ~16:36 UTC, >12h later
  per rule (b)): still zero drift** — last successful `Deploy to
  Production` run is still the same 2026-08-01T18:20:33Z push at sha
  `8dde7500` (confirmed via `gh run list --workflow=deploy.yml`), HEAD
  (`3e705f8`) unverified, no `actions: write` grant (a live `gh workflow
  run deploy.yml --ref main` still fails with the same `HTTP 403: Resource
  not accessible by integration`), no PAT (`gh secret list` also 403s),
  backlog still 91 unapproved `todo` tasks, task 345 still
  `blocked`/correct, task 348 still `todo`/unapproved. Epics 7/9/15/16/18
  all still `approved` this check (no oscillation since the 01:54 UTC
  check), each still with its linked `done` tasks intact. No approved
  `todo` task and no approved epic-awaiting-breakdown existed to build
  (all 5 approved epics already have ≥1 linked task), so nothing was
  shippable this run; the 91-item unapproved backlog was left as-is per
  the "optional, not mandatory" guidance above. **Re-check (2026-09-02,
  ~06:25 UTC, >12h later per rule (b)): still zero drift** — last
  successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml`), HEAD (`e9f88a6`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 all still `approved` this check (no oscillation since
  the 16:36 UTC check), each still with its linked `done` tasks intact. No
  approved `todo` task and no approved epic-awaiting-breakdown existed to
  build (all 5 approved epics already have ≥1 linked task), so nothing was
  shippable this run; the 91-item unapproved backlog was left as-is per
  the "optional, not mandatory" guidance above. **Re-check (2026-09-02,
  ~11:32 UTC): still zero drift on the deploy itself** — last successful
  `Deploy to Production` run is still the same 2026-08-01T18:20:33Z push at
  sha `8dde7500` (confirmed via `gh run list --workflow=deploy.yml`), HEAD
  (`5a2c867`) unverified, no `actions: write` grant (a live `gh workflow
  run deploy.yml --ref main` still fails with the same `HTTP 403: Resource
  not accessible by integration`), no PAT (`gh secret list` also 403s),
  backlog still 91 unapproved `todo` tasks, task 345 still
  `blocked`/correct, task 348 still `todo`/unapproved. Epics 7/9/15/16/18
  flipped back to `proposed` this check (reversing the 06:25 UTC check's
  `approved` reading, linked `done` tasks intact throughout, confirmed via
  a direct join query showing 2/2/2/1/2 linked tasks all `done`) — another
  oscillation event per task 348. This is being folded into a commit now
  under the narrowed rule because it's >12h since the last *recorded
  oscillation note* (2026-08-26 ~06:31 UTC, the last time this paragraph
  recorded an actual flip rather than "no oscillation"), even though it's
  only ~5h since the last routine zero-drift timestamp (~06:25 UTC same
  day) — the two triggers are tracked independently, exactly as in the
  2026-08-21 ~23:14 UTC, 2026-08-23 ~02:14 UTC, and 2026-08-24 ~20:16 UTC
  precedents. No approved `todo` task existed and no approved
  epic-awaiting-breakdown existed to build (all epics are currently
  `proposed`, not `approved`, so step 3's breakdown trigger doesn't apply
  either), so nothing was shippable; the 91-item unapproved backlog was
  left as-is per the "optional, not mandatory" guidance above. **Re-check
  (2026-09-03, ~01:16 UTC, >12h later per rule (b)): still zero drift on
  the deploy itself** — last successful `Deploy to Production` run is
  still the same 2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via
  `gh run list --workflow=deploy.yml`), HEAD (`cefae80`) unverified, no
  `actions: write` grant (a live `gh workflow run deploy.yml --ref main`
  still fails with the same `HTTP 403: Resource not accessible by
  integration`), no PAT (`gh secret list` also 403s), backlog still 91
  unapproved `todo` tasks, task 345 still `blocked`/correct, task 348 still
  `todo`/unapproved. Epics 7/9/15/16/18 flipped back to `approved` this
  check (reversing the 11:32 UTC check's `proposed` reading, linked `done`
  ...
  348, folded in here under the same >12h rule (last recorded oscillation
  note was also 2026-09-02 ~11:32 UTC) rather than as its own commit. No
  approved `todo` task existed and no approved epic-awaiting-breakdown
  existed to build (all 5 approved epics already have ≥1 linked task), so
  nothing was shippable this run; the 91-item unapproved backlog was left
  as-is per the "optional, not mandatory" guidance above. **Re-check
  (2026-09-03, ~14:34 UTC, >12h later per rule (b)): still zero drift** —
  last successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml`), HEAD (`ae46268`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 all still `approved` this check (no oscillation since
  the 01:16 UTC check), each still with its linked `done` tasks intact
  (2/2/2/1/2). No approved `todo` task and no approved epic-awaiting-
  ...
  **Re-check (2026-09-04, ~05:07 UTC, >12h later per rule (b)): still zero
  ...
  doesn't apply either), so nothing was shippable; the 91-item unapproved
  backlog was left as-is per the "optional, not mandatory" guidance above.
  **Re-check (2026-09-04, ~17:14 UTC, >12h later per rule (b)): still zero
  drift on the deploy itself** — last successful `Deploy to Production` run
  is still the same 2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed
  via `gh run list --workflow=deploy.yml --json databaseId,displayTitle,
  createdAt,conclusion,headSha`), HEAD (`c23ccbf`) unverified, no
  `actions: write` grant (a live `gh workflow run deploy.yml --ref main`
  still fails with the same `HTTP 403: Resource not accessible by
  integration`), no PAT (`gh secret list` also 403s), backlog still 91
  unapproved `todo` tasks, task 345 still `blocked`/correct, task 348 still
  `todo`/unapproved. Epics 7/9/15/16/18 flipped back to `approved` this
  check (reversing the 05:07 UTC check's `proposed` reading, linked `done`
  tasks intact throughout, confirmed via a direct join query showing
  2/2/2/1/2 linked tasks all `done`) — another oscillation event per task
  348, folded in here under the same >12h rule (last recorded oscillation
  note was also 2026-09-04 ~05:07 UTC) rather than as its own commit. No
  approved `todo` task existed and no approved epic-awaiting-breakdown
  existed to build (all 5 approved epics already have ≥1 linked task), so
  nothing was shippable this run; the 91-item unapproved backlog was left
  as-is per the "optional, not mandatory" guidance above. **Re-check
  (2026-09-05, ~05:47 UTC, >12h later per rule (b)): still zero drift on
  the deploy itself** — last successful `Deploy to Production` run is
  still the same 2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed
  via `gh run list --workflow=deploy.yml --json databaseId,displayTitle,
  createdAt,conclusion,headSha`), HEAD (`7bc482b`) unverified, no
  `actions: write` grant (a live `gh workflow run deploy.yml --ref main`
  still fails with the same `HTTP 403: Resource not accessible by
  integration`), no PAT (`gh secret list` also 403s), backlog still 91
  unapproved `todo` tasks, task 345 still `blocked`/correct, task 348 still
  `todo`/unapproved. Epics 7/9/15/16/18 flipped back to `proposed` this
  check (reversing the 17:14 UTC check's `approved` reading, linked `done`
  tasks intact throughout, confirmed via a direct join query showing
  2/2/2/1/2 linked tasks all `done`) — another oscillation event per task
  348, folded in here under the same >12h rule (last recorded oscillation
  note was also 2026-09-04 ~17:14 UTC) rather than as its own commit. No
  approved `todo` task existed and no approved epic-awaiting-breakdown
  existed to build (all epics are currently `proposed`, not `approved`, so
  step 3's breakdown trigger doesn't apply either), so nothing was
  shippable this run; the 91-item unapproved backlog was left as-is per
  the "optional, not mandatory" guidance above. **Re-check (2026-09-05,
  ~18:15 UTC, >12h later per rule (b)): still zero drift on the deploy
  itself** — last successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml --json databaseId,displayTitle,createdAt,
  conclusion,headSha`), HEAD (`1be4525`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 flipped back to `approved` this check (reversing the
  05:47 UTC check's `proposed` reading, linked `done` tasks intact
  throughout) — another oscillation event per task 348, folded in here
  under the same >12h rule (last recorded oscillation note was also
  2026-09-05 ~05:47 UTC) rather than as its own commit. No approved `todo`
  task existed and no approved epic-awaiting-breakdown existed to build
  (all 5 approved epics already have ≥1 linked task), so nothing was
  shippable this run; the 91-item unapproved backlog was left as-is per
  the "optional, not mandatory" guidance above. **Re-check (2026-09-06,
  ~06:29 UTC, >12h later per rule (b)): still zero drift on the deploy
  itself** — last successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml --json databaseId,displayTitle,createdAt,
  conclusion,headSha`), HEAD (`0817e8a`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 flipped back to `proposed` this check (reversing the
  18:15 UTC check's `approved` reading, linked `done` tasks intact
  throughout) — another oscillation event per task 348, folded in here
  under the same >12h rule (last recorded oscillation note was also
  2026-09-05 ~18:15 UTC) rather than as its own commit. No approved `todo`
  task existed and no approved epic-awaiting-breakdown existed to build
  (all epics are currently `proposed`, not `approved`, so step 3's
  breakdown trigger doesn't apply either), so nothing was shippable this
  run; the 91-item unapproved backlog was left as-is per the "optional,
  not mandatory" guidance above. **Re-check (2026-09-06, ~18:51 UTC, >12h
  later per rule (b), and >12h since the last recorded oscillation note at
  ~06:29 UTC the same day): still zero drift on the deploy itself** — last
  successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml --json databaseId,displayTitle,createdAt,
  conclusion,headSha`), HEAD (`13d6d59`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 flipped back to `approved` this check (reversing the
  06:29 UTC check's `proposed` reading, linked `done` tasks intact
  throughout, confirmed via a direct join query showing 2/2/2/1/2 linked
  tasks all `done`) — another oscillation event per task 348, folded in
  here under the same >12h rule rather than as its own commit. No approved
  `todo` task existed and no approved epic-awaiting-breakdown existed to
  build (all 5 approved epics already have ≥1 linked task), so nothing was
  shippable this run; the 91-item unapproved backlog was left as-is per
  the "optional, not mandatory" guidance above. **Re-check (2026-09-07,
  ~09:42 UTC, >12h later per rule (b)): still zero drift** — last
  successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml --json databaseId,displayTitle,createdAt,
  conclusion,headSha`), HEAD (`279fb72`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 all still `approved` this check (no oscillation since
  the 18:51 UTC check), each still with its linked `done` tasks intact
  (2/2/2/1/2). No approved `todo` task and no approved epic-awaiting-
  breakdown existed to build (all 5 approved epics already have ≥1 linked
  task), so nothing was shippable this run; the 91-item unapproved backlog
  was left as-is per the "optional, not mandatory" guidance above.
  **Re-check (2026-09-07, ~22:00 UTC, >12h later per rule (b)): still zero
  drift** — last successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml --json databaseId,displayTitle,createdAt,
  conclusion,headSha`), HEAD (`d75530b`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 all still `approved` this check (no oscillation since
  the 09:42 UTC check), each still with its linked `done` tasks intact. No
  approved `todo` task and no approved epic-awaiting-breakdown existed to
  build (all 5 approved epics already have ≥1 linked task), so nothing was
  shippable this run; the 91-item unapproved backlog was left as-is per
  the "optional, not mandatory" guidance above. **Re-check (2026-09-08,
  ~12:52 UTC, >12h later per rule (b)): still zero drift** — last
  successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml --json databaseId,displayTitle,createdAt,
  conclusion,headSha`), HEAD (`5d913e2`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 all still `approved` this check (no oscillation since
  the 22:00 UTC check), each still with its linked `done` tasks intact
  (2/2/2/1/2). No approved `todo` task and no approved epic-awaiting-
  breakdown existed to build (all 5 approved epics already have ≥1 linked
  task), so nothing was shippable this run; the 91-item unapproved backlog
  was left as-is per the "optional, not mandatory" guidance above.
  **Re-check (2026-09-08, ~19:40 UTC): still zero drift on the deploy
  itself** — last successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml --json databaseId,displayTitle,createdAt,
  conclusion,headSha`), HEAD (`c735678`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 flipped back to `proposed` this check (reversing the
  12:52 UTC check's `approved` reading, linked `done` tasks intact
  throughout, confirmed via a direct join query showing 2/2/2/1/2 linked
  tasks all `done`) — another oscillation event per task 348. This is being
  folded into a commit now under the narrowed rule because it's >12h since
  the last *recorded oscillation note* (2026-09-06 ~18:51 UTC, the last
  time this paragraph recorded an actual flip rather than "no
  oscillation"), even though it's only ~7h since the last routine
  zero-drift timestamp (~12:52 UTC the same day) — the two triggers are
  tracked independently, exactly as in the 2026-08-21 ~23:14 UTC and
  2026-09-02 ~11:32 UTC precedents. No approved `todo` task existed and no
  approved epic-awaiting-breakdown existed to build (all epics are
  currently `proposed`, not `approved`, so step 3's breakdown trigger
  doesn't apply either), so nothing was shippable; the 91-item unapproved
  backlog was left as-is per the "optional, not mandatory" guidance above.
  **Re-check (2026-09-09, ~09:10 UTC, >12h later per rule (b)): still zero
  drift** — last successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml --json databaseId,displayTitle,createdAt,
  conclusion,headSha`), HEAD (`d48c31d`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 all still `approved` this check (no oscillation since
  the 19:40 UTC check), each still with its linked `done` tasks intact
  (2/2/2/1/2). No approved `todo` task and no approved epic-awaiting-
  breakdown existed to build (all 5 approved epics already have ≥1 linked
  task), so nothing was shippable this run; the 91-item unapproved backlog
  was left as-is per the "optional, not mandatory" guidance above.
  **Re-check (2026-09-09, ~22:03 UTC, >12h later per rule (b)): still zero
  drift** — last successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml --json databaseId,displayTitle,createdAt,
  conclusion,headSha`), HEAD (`c64be87`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 all still `approved` this check (no oscillation since
  the 09:10 UTC check), each still with its linked `done` tasks intact
  (2/2/2/1/2). No approved `todo` task and no approved epic-awaiting-
  breakdown existed to build (all 5 approved epics already have ≥1 linked
  task), so nothing was shippable this run; the 91-item unapproved backlog
  was left as-is per the "optional, not mandatory" guidance above.
  **Re-check (2026-09-10, ~13:34 UTC, >12h later per rule (b)): still zero
  drift** — last successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml --json databaseId,displayTitle,createdAt,
  conclusion,headSha`), HEAD (`b2f2279`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 all still `approved` this check (no oscillation since
  the 22:03 UTC check), each still with its linked `done` tasks intact.
  No approved `todo` task and no approved epic-awaiting-breakdown existed
  to build (all 5 approved epics already have ≥1 linked task), so nothing
  was shippable this run; the 91-item unapproved backlog was left as-is
  per the "optional, not mandatory" guidance above. **Re-check (2026-09-11,
  ~06:12 UTC, >12h later per rule (b)): still zero drift** — last
  successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml --json databaseId,displayTitle,createdAt,
  conclusion,headSha`), HEAD (`9ab17b0`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 all still `approved` this check (no oscillation since
  the 13:34 UTC check), each still with its linked `done` tasks intact
  (2/2/2/1/2). No approved `todo` task and no approved epic-awaiting-
  breakdown existed to build (all 5 approved epics already have ≥1 linked
  task), so nothing was shippable this run; the 91-item unapproved backlog
  was left as-is per the "optional, not mandatory" guidance above.
  **Re-check (2026-09-11, ~18:28 UTC, >12h later per rule (b)): still zero
  drift** — last successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml --json databaseId,displayTitle,createdAt,
  conclusion,headSha`), HEAD (`dc27ed9`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 all still `approved` this check (no oscillation since
  the 06:12 UTC check), each still with its linked `done` tasks intact
  (2/2/2/1/2). No approved `todo` task and no approved epic-awaiting-
  breakdown existed to build (all 5 approved epics already have ≥1 linked
  task), so nothing was shippable this run; the 91-item unapproved backlog
  was left as-is per the "optional, not mandatory" guidance above.
  **Re-check (2026-09-12, ~09:36 UTC, >12h later per rule (b)): still zero
  drift** — last successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml --json databaseId,displayTitle,createdAt,
  conclusion,headSha`), HEAD (`37b59c7`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 all still `approved` this check (no oscillation since
  the 18:28 UTC check), each still with its linked `done` tasks intact.
  No approved `todo` task and no approved epic-awaiting-breakdown existed
  to build (all 5 approved epics already have ≥1 linked task), so nothing
  was shippable this run; the 91-item unapproved backlog was left as-is
  per the "optional, not mandatory" guidance above. **Re-check (2026-09-12,
  ~22:43 UTC, >12h later per rule (b)): still zero drift** — last
  successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml --json databaseId,displayTitle,createdAt,
  conclusion,headSha`), HEAD (`d2ef21d`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 all still `approved` this check (no oscillation since
  the 09:36 UTC check), each still with its linked `done` tasks intact
  (2/2/2/1/2). No approved `todo` task and no approved epic-awaiting-
  breakdown existed to build (all 5 approved epics already have ≥1 linked
  task), so nothing was shippable this run; the 91-item unapproved backlog
  was left as-is per the "optional, not mandatory" guidance above.
  **Re-check (2026-09-13, ~14:24 UTC, >12h later per rule (b)): still zero
  drift** — last successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml --json databaseId,displayTitle,createdAt,
  conclusion,headSha`), HEAD (`4a36494`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 all still `approved` this check (no oscillation since
  the 22:43 UTC check), each still with its linked `done` tasks intact. No
  approved `todo` task and no approved epic-awaiting-breakdown existed to
  build (all 5 approved epics already have ≥1 linked task), so nothing was
  shippable this run; the 91-item unapproved backlog was left as-is per
  the "optional, not mandatory" guidance above. **Re-check (2026-09-14,
  ~06:31 UTC, >12h later per rule (b)): still zero drift** — last
  successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml --json databaseId,displayTitle,createdAt,
  conclusion,headSha`), HEAD (`461124a`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 all still `approved` this check (no oscillation since
  the 14:24 UTC check), each still with its linked `done` tasks intact
  (2/2/2/1/2). No approved `todo` task and no approved epic-awaiting-
  breakdown existed to build (all 5 approved epics already have ≥1 linked
  task), so nothing was shippable this run; the 91-item unapproved backlog
  was left as-is per the "optional, not mandatory" guidance above.
  **Re-check (2026-09-14, ~18:47 UTC, >12h later per rule (b)): still zero
  drift** — last successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml --json databaseId,displayTitle,createdAt,
  conclusion,headSha`), HEAD (`8cb89b4`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 all still `approved` this check (no oscillation since
  the 06:31 UTC check), each still with its linked `done` tasks intact
  (2/2/2/1/2). No approved `todo` task and no approved epic-awaiting-
  breakdown existed to build (all 5 approved epics already have ≥1 linked
  task), so nothing was shippable this run; the 91-item unapproved backlog
  was left as-is per the "optional, not mandatory" guidance above.
  **Re-check (2026-09-14, ~22:07 UTC): still zero drift on the deploy
  itself** — last successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml --json databaseId,displayTitle,createdAt,
  conclusion,headSha`), HEAD (`cab2b12`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 flipped back to `proposed` this check (reversing the
  18:47 UTC check's `approved` reading, linked `done` tasks intact
  throughout, confirmed via a direct join query showing 2/2/2/1/2 linked
  tasks all `done`) — another oscillation event per task 348. This is being
  folded into a commit now under the narrowed rule because it's >12h since
  the last *recorded oscillation note* (2026-09-08 ~19:40 UTC, the last
  time this paragraph recorded an actual flip rather than "no
  oscillation"), even though it's only ~3h20m since the last routine
  zero-drift timestamp (~18:47 UTC the same day) — the two triggers are
  tracked independently, exactly as in the 2026-08-21 ~23:14 UTC and
  2026-09-08 ~19:40 UTC precedents. No approved `todo` task existed and no
  approved epic-awaiting-breakdown existed to build (all epics are
  currently `proposed`, not `approved`, so step 3's breakdown trigger
  doesn't apply either), so nothing was shippable; the 91-item unapproved
  backlog was left as-is per the "optional, not mandatory" guidance above.
  **Re-check (2026-09-15, ~10:13 UTC, >12h later per rule (b)): still zero
  drift on the deploy itself** — last successful `Deploy to Production` run
  is still the same 2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed
  via `gh run list --workflow=deploy.yml --json databaseId,displayTitle,
  createdAt,conclusion,headSha`), HEAD (`7ad353b`) unverified, no
  `actions: write` grant (a live `gh workflow run deploy.yml --ref main`
  still fails with the same `HTTP 403: Resource not accessible by
  integration`), no PAT (`gh secret list` also 403s), backlog still 91
  unapproved `todo` tasks, task 345 still `blocked`/correct, task 348 still
  `todo`/unapproved. Epics 7/9/15/16/18 flipped back to `approved` this
  check (reversing the 22:07 UTC check's `proposed` reading, linked `done`
  tasks intact throughout, confirmed via a direct join query showing
  2/2/2/1/2 linked tasks all `done`) — another oscillation event per task
  348, folded in here under the same >12h rule (last recorded oscillation
  note was also 2026-09-14 ~22:07 UTC) rather than as its own commit. No
  approved `todo` task existed and no approved epic-awaiting-breakdown
  existed to build (all 5 approved epics already have ≥1 linked task), so
  nothing was shippable this run; the 91-item unapproved backlog was left
  as-is per the "optional, not mandatory" guidance above. **Re-check
  (2026-09-15, ~23:44 UTC, >12h later per rule (b)): still zero drift** —
  last successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml --json databaseId,displayTitle,createdAt,
  conclusion,headSha`), HEAD (`f4aafc7`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 all still `approved` this check (no oscillation since
  the 10:13 UTC check), each still with its linked `done` tasks intact
  (2/2/2/1/2). No approved `todo` task and no approved epic-awaiting-
  breakdown existed to build (all 5 approved epics already have ≥1 linked
  task), so nothing was shippable this run; the 91-item unapproved backlog
  was left as-is per the "optional, not mandatory" guidance above.
  **Re-check (2026-09-16, ~01:41 UTC): still zero drift on the deploy
  itself** — last successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml --json databaseId,displayTitle,createdAt,
  conclusion,headSha`), HEAD (`1bede77`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 flipped back to `proposed` this check (reversing the
  23:44 UTC check's `approved` reading, linked `done` tasks intact
  throughout, confirmed via a direct join query showing 2/2/2/1/2 linked
  tasks all `done`) — another oscillation event per task 348. This is being
  folded into a commit now under the narrowed rule because it's >12h since
  the last *recorded oscillation note* (2026-09-15 ~10:13 UTC, the last
  time this paragraph recorded an actual flip rather than "no
  oscillation"), even though it's only ~2h since the last routine
  zero-drift timestamp (~23:44 UTC the day before) — the two triggers are
  tracked independently, exactly as in the 2026-08-21 ~23:14 UTC and
  2026-09-14 ~22:07 UTC precedents. No approved `todo` task existed and no
  approved epic-awaiting-breakdown existed to build (all epics are
  currently `proposed`, not `approved`, so step 3's breakdown trigger
  doesn't apply either), so nothing was shippable; the 91-item unapproved
  backlog was left as-is per the "optional, not mandatory" guidance above.
  **Re-check (2026-09-16, ~16:49 UTC, >12h later per rule (b)): still zero
  drift on the deploy itself** — last successful `Deploy to Production` run
  is still the same 2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed
  via `gh run list --workflow=deploy.yml --json databaseId,displayTitle,
  createdAt,conclusion,headSha`), HEAD (`62f0fb8`) unverified, no
  `actions: write` grant (a live `gh workflow run deploy.yml --ref main`
  still fails with the same `HTTP 403: Resource not accessible by
  integration`), no PAT (`gh secret list` also 403s), backlog still 91
  unapproved `todo` tasks, task 345 still `blocked`/correct, task 348 still
  `todo`/unapproved. Epics 7/9/15/16/18 flipped back to `approved` this
  check (reversing the 01:41 UTC check's `proposed` reading, linked `done`
  tasks intact throughout, confirmed via a direct join query showing
  2/2/2/1/2 linked tasks all `done`) — another oscillation event per task
  348, folded in here under the same >12h rule (last recorded oscillation
  note was also 2026-09-16 ~01:41 UTC) rather than as its own commit. No
  approved `todo` task existed and no approved epic-awaiting-breakdown
  existed to build (all 5 approved epics already have ≥1 linked task), so
  nothing was shippable this run; the 91-item unapproved backlog was left
  as-is per the "optional, not mandatory" guidance above. **Re-check
  (2026-09-17, ~05:26 UTC, >12h later per rule (b)): still zero drift** —
  last successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml --json databaseId,displayTitle,createdAt,
  conclusion,headSha`), HEAD (`8b68e2b`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 all still `approved` this check (no oscillation since
  the 16:49 UTC check), each still with its linked `done` tasks intact
  (2/2/2/1/2). No approved `todo` task and no approved epic-awaiting-
  breakdown existed to build (all 5 approved epics already have ≥1 linked
  task), so nothing was shippable this run; the 91-item unapproved backlog
  was left as-is per the "optional, not mandatory" guidance above.
  **Re-check (2026-09-17, ~18:45 UTC, >12h later per rule (b)): still zero
  drift** — last successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml --json databaseId,displayTitle,createdAt,
  conclusion,headSha`), HEAD (`ffe66bc`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 all still `approved` this check (no oscillation since
  the 05:26 UTC check), each still with its linked `done` tasks intact
  (2/2/2/1/2). No approved `todo` task and no approved epic-awaiting-
  breakdown existed to build (all 5 approved epics already have ≥1 linked
  task), so nothing was shippable this run; the 91-item unapproved backlog
  was left as-is per the "optional, not mandatory" guidance above.
  **Re-check (2026-09-18, ~06:48 UTC, >12h later per rule (b)): still zero
  drift** — last successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml --json databaseId,displayTitle,createdAt,
  conclusion,headSha`), HEAD (`7604303`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 all still `approved` this check (no oscillation since
  the 18:45 UTC check). No approved `todo` task and no approved
  epic-awaiting-breakdown existed to build (all 5 approved epics already
  have ≥1 linked task), so nothing was shippable this run; the 91-item
  unapproved backlog was left as-is per the "optional, not mandatory"
  guidance above. **Re-check (2026-09-18, ~18:38 UTC): still zero drift on
  the deploy itself** — last successful `Deploy to Production` run is
  still the same 2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via
  `gh run list --workflow=deploy.yml --json databaseId,displayTitle,
  createdAt,conclusion,headSha`), HEAD (`ac1f6b8`) unverified, no
  `actions: write` grant (a live `gh workflow run deploy.yml --ref main`
  still fails with the same `HTTP 403: Resource not accessible by
  integration`), no PAT (`gh secret list` also 403s), backlog still 91
  unapproved `todo` tasks, task 345 still `blocked`/correct, task 348 still
  `todo`/unapproved. Epics 7/9/15/16/18 flipped back to `proposed` this
  check (reversing the 06:48 UTC check's `approved` reading, linked `done`
  tasks intact throughout, confirmed via a direct join query showing
  2/2/2/1/2 linked tasks all `done`) — another oscillation event per task
  348. This is being folded into a commit now under the narrowed rule
  because it's >12h since the last *recorded oscillation note* (2026-09-16
  ~16:49 UTC, the last time this paragraph recorded an actual flip rather
  than "no oscillation"), even though it's only ~11h50m since the last
  routine zero-drift timestamp (~06:48 UTC the same day) — the two
  triggers are tracked independently, exactly as in the 2026-08-21
  ~23:14 UTC and 2026-09-16 ~01:41 UTC precedents. No approved `todo` task
  existed and no approved epic-awaiting-breakdown existed to build (all
  ...
  have ≥1 linked task), so nothing was shippable this run; the 91-item
  unapproved backlog was left as-is per the "optional, not mandatory"
  guidance above. **Re-check (2026-09-19, ~23:32 UTC, >12h later per rule
  (b)): still zero drift** — last successful `Deploy to Production` run is
  still the same 2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via
  `gh run list --workflow=deploy.yml --json databaseId,displayTitle,
  createdAt,conclusion,headSha`), HEAD (`f0376eb`) unverified, no
  `actions: write` grant (a live `gh workflow run deploy.yml --ref main`
  still fails with the same `HTTP 403: Resource not accessible by
  integration`), no PAT (`gh secret list` also 403s), backlog still 91
  unapproved `todo` tasks, task 345 still `blocked`/correct, task 348 still
  `todo`/unapproved. Epics 7/9/15/16/18 all still `approved` this check (no
  oscillation since the 10:53 UTC check), each still with its linked
  `done` tasks intact (2/2/2/1/2). No approved `todo` task and no approved
  epic-awaiting-breakdown existed to build (all 5 approved epics already
  have ≥1 linked task), so nothing was shippable this run; the 91-item
  unapproved backlog was left as-is per the "optional, not mandatory"
  guidance above. **Re-check (2026-09-20, ~12:00 UTC, >12h later per rule
  (b)): still zero drift** — last successful `Deploy to Production` run is
  still the same 2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via
  `gh run list --workflow=deploy.yml --json databaseId,displayTitle,
  createdAt,conclusion,headSha`), HEAD (`8babc9d`) unverified, no
  `actions: write` grant (a live `gh workflow run deploy.yml --ref main`
  still fails with the same `HTTP 403: Resource not accessible by
  integration`), no PAT (`gh secret list` also 403s), backlog still 91
  unapproved `todo` tasks, task 345 still `blocked`/correct, task 348 still
  `todo`/unapproved. Epics 7/9/15/16/18 all still `approved` this check (no
  oscillation since the 23:32 UTC check), each still with its linked `done`
  tasks intact (2/2/2/1/2). No approved `todo` task and no approved
  epic-awaiting-breakdown existed to build (all 5 approved epics already
  have ≥1 linked task), so nothing was shippable this run; the 91-item
  unapproved backlog was left as-is per the "optional, not mandatory"
  guidance above. **Re-check (2026-09-20, ~22:50 UTC): still zero drift on
  the deploy itself** — last successful `Deploy to Production` run is
  still the same 2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via
  `gh run list --workflow=deploy.yml --json databaseId,displayTitle,
  createdAt,conclusion,headSha`), no `actions: write` grant (a live
  `gh workflow run deploy.yml --ref main` still fails with the same
  `HTTP 403: Resource not accessible by integration`), no PAT (`gh secret
  list` also 403s), backlog still 91 unapproved `todo` tasks, task 345
  still `blocked`/correct, task 348 still `todo`/unapproved. Epics
  7/9/15/16/18 flipped back to `proposed` this check (reversing the
  12:00 UTC check's `approved` reading, linked `done` tasks intact
  throughout, confirmed via a direct join query showing 2/2/2/1/2 linked
  tasks all `done`) — another oscillation event per task 348. This is being
  folded into a commit now under the narrowed rule because it's >12h since
  the last *recorded oscillation note* (2026-09-19 ~10:53 UTC, the last
  time this paragraph recorded an actual flip rather than "no
  oscillation"), even though it's only ~10h50m since the last routine
  zero-drift timestamp (~12:00 UTC the same day) — the two triggers are
  tracked independently, exactly as in the 2026-08-21 ~23:14 UTC and
  2026-09-18 ~18:38 UTC precedents. No approved `todo` task existed and no
  approved epic-awaiting-breakdown existed to build (all epics are
  currently `proposed`, not `approved`, so step 3's breakdown trigger
  doesn't apply either), so nothing was shippable; the 91-item unapproved
  backlog was left as-is per the "optional, not mandatory" guidance above.
  **Re-check (2026-09-21, ~10:55 UTC, >12h later per rule (b)): still zero
  drift on the deploy itself** — last successful `Deploy to Production` run
  is still the same 2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed
  via `gh run list --workflow=deploy.yml --json databaseId,displayTitle,
  createdAt,conclusion,headSha`), HEAD (`ff6e6fb`) unverified, no
  `actions: write` grant (a live `gh workflow run deploy.yml --ref main`
  still fails with the same `HTTP 403: Resource not accessible by
  integration`), no PAT (`gh secret list` also 403s), backlog still 91
  unapproved `todo` tasks, task 345 still `blocked`/correct, task 348 still
  `todo`/unapproved. Epics 7/9/15/16/18 flipped back to `approved` this
  check (reversing the 22:50 UTC check's `proposed` reading, linked `done`
  tasks intact throughout, confirmed via a direct join query showing
  2/2/2/1/2 linked tasks all `done`) — another oscillation event per task
  348, folded in here under the same >12h rule (last recorded oscillation
  note was also 2026-09-20 ~22:50 UTC) rather than as its own commit. No
  approved `todo` task existed and no approved epic-awaiting-breakdown
  existed to build (all 5 approved epics already have ≥1 linked task), so
  nothing was shippable this run; the 91-item unapproved backlog was left
  as-is per the "optional, not mandatory" guidance above. **Re-check
  (2026-09-21, ~23:36 UTC, >12h later per rule (b)): still zero drift** —
  last successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml --json databaseId,displayTitle,createdAt,
  conclusion,headSha`), HEAD (`90d076b`) unverified, no `actions: write`
  grant (a live `gh workflow run deploy.yml --ref main` still fails with
  the same `HTTP 403: Resource not accessible by integration`), no PAT
  (`gh secret list` also 403s), backlog still 91 unapproved `todo` tasks,
  task 345 still `blocked`/correct, task 348 still `todo`/unapproved.
  Epics 7/9/15/16/18 all still `approved` this check (no oscillation since
  the 10:55 UTC check), each still with its linked `done` tasks intact
  (2/2/2/1/2). No approved `todo` task and no approved epic-awaiting-
  breakdown existed to build (all 5 approved epics already have ≥1 linked
  task), so nothing was shippable this run; the 91-item unapproved backlog
  was left as-is per the "optional, not mandatory" guidance above.
  **Re-check (2026-09-22, ~12:16 UTC, >12h later per rule (b)): still zero
  drift** — last successful `Deploy to Production` run is still the same
  2026-08-01T18:20:33Z push at sha `8dde7500` (confirmed via `gh run list
  --workflow=deploy.yml --json databaseId,displayTitle,createdAt,
  conclusion,headSha`), HEAD (`b89aca3`) unverified (`gh api .../status`
  returns `state: pending`, 0 statuses), no `actions: write` grant (a live
  `gh workflow run deploy.yml --ref main` still fails with the same
  `HTTP 403: Resource not accessible by integration`), no PAT (`gh secret
  list` also 403s), backlog still 91 unapproved `todo` tasks, task 345
  still `blocked`/correct, task 348 still `todo`/unapproved. Epics
  7/9/15/16/18 all still `approved` this check (no oscillation since the
  23:36 UTC check), each still with its linked `done` tasks intact
  (2/2/2/1/2, confirmed via a direct query against the synced
  `tshirt_store` sqlite file — see the `DB_DATABASE` gotcha entry below for
  why that's the file to query, not `database/database.sqlite`). No
  approved `todo` task and no approved epic-awaiting-breakdown existed to
  build (all 5 approved epics already have ≥1 linked task), so nothing was
  shippable this run; the 91-item unapproved backlog was left as-is per
  the "optional, not mandatory" guidance above. **Re-check (2026-09-22,
  ~22:56 UTC): still zero drift on the deploy itself** — last successful
  `Deploy to Production` run is still the same 2026-08-01T18:20:33Z push
  at sha `8dde7500` (confirmed via `gh run list --workflow=deploy.yml
  --json databaseId,displayTitle,createdAt,conclusion,headSha`), HEAD
  (`8ec07b7`) unverified (`gh api .../status` returns `state: pending`),
  no `actions: write` grant (a live `gh workflow run deploy.yml --ref
  main` still fails with the same `HTTP 403: Resource not accessible by
  integration`), no PAT (`gh secret list` also 403s), backlog still 91
  unapproved `todo` tasks, task 345 still `blocked`/correct, task 348
  still `todo`/unapproved. Epics 7/9/15/16/18 flipped back to `proposed`
  this check (reversing the 12:16 UTC check's `approved` reading, linked
  `done` tasks intact throughout, confirmed via a direct query against the
  synced `tshirt_store` sqlite file showing 2/2/2/1/2 linked tasks all
  `done`) — another oscillation event per task 348. This is being folded
  into a commit now under the narrowed rule because it's >12h since the
  last *recorded oscillation note* (2026-09-20 ~22:50 UTC, the last time
  this paragraph recorded an actual flip rather than "no oscillation"),
  even though it's only ~10h40m since the last routine zero-drift
  timestamp (~12:16 UTC the same day) — the two triggers are tracked
  independently, exactly as in the 2026-08-21 ~23:14 UTC and 2026-09-20
  ~22:50 UTC precedents. No approved `todo` task existed and no approved
  epic-awaiting-breakdown existed to build (all epics are currently
  `proposed`, not `approved`, so step 3's breakdown trigger doesn't apply
  either), so nothing was shippable; the 91-item unapproved backlog was
