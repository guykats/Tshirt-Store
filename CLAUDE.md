...
- **`deploy.yml` silently stopped auto-deploying on 2026-08-01 — still broken
  ...
  2026-08-13 (last re-checked ~16:32 UTC, still zero drift vs. the ~11:38 UTC
  ...
  back at `approved` (with their linked tasks intact) and task 348 still
  `todo`/unapproved — the same task-348 oscillation already documented
  below (the prior 14:44 UTC check had all 12 at `proposed`; this run swung
  back within under an hour), not a new finding — still no
  approved-epic-awaiting-breakdown work (all 5 approved epics already have
  linked tasks) and no approved todo task to build this run either) has
  re-checked
